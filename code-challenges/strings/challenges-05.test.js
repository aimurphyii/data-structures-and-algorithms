...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    let erase = element.split(/\s/g);
    // console.log(erase);
    // console.log(erase.slice(2));
    let arr = erase.slice(2);
    let list = arr.join();
    // console.log(list);
    result.push(list.replace(/,/g,' '));
  });
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(element => {
    // let erase = element.replace(/\d*\d\s\w+-*\w+\s/, '');
    // console.log(erase);
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(element => {
    // console.log(element);
    // console.log(element.match(/\w+$/));
    // console.log((element.match(/\w+$/))[0])
    result.push((element.match(/^\w+-*\w+/))[0]);
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters>str.length){
    let str = '';
    return str;
  }else if (numberOfCharacters<=str.length&&numberOfCharacters>0){
  let arr = str.split('');
  // console.log(arr);
  let short = arr.splice(arr.length-numberOfCharacters,numberOfCharacters);
  // console.log(short);
  // console.log(arr);
  let answr = arr.join();
  let ret = answr.replace(/,/gi, '');
...
const totalSumCSV = (str) => {
  ...
  for (let i=0;i<arr.length;i++){
    numbarr.push(parseInt(arr[i]));
  }
  // console.log(numbarr);
  let sum = numbarr.reduce(function(a, b) { return a + b; });
  // console.log(sum);
  return total+sum
  // return total;
};
...
const removeVowels = (str) => {
  // Solution code here...
  let novowels = str.replace(/[aeiou]/ig, '');
  return novowels;
};
...
const extractVowels = (str) => {
  // Solution code here...
  let findvowels = str.match(/[aeiou]/ig);
  let novowels = str.replace(/[aeiou]/ig, '');
  let found = findvowels.sort().join();
  // console.log(found);
  let final = found.replace(/,/ig,'');
  // console.log(final);
  let arr = [];
  arr.push(novowels);
  arr.push(final);
