...
const totalCharacters = (arr) => {
  ...
  let howmany= [];
  arr.forEach(obj=>{
    howmany.push(obj.name);
    if(obj.spouse!==null&&howmany.includes(obj.spouse)===false){
    howmany.push(obj.spouse);
    };
    if(obj.children!==[]){
      for (let i=0;i<obj.children.length;i++){
        if(howmany.includes(obj.children[i])===false){
        howmany.push(obj.children[i]);}
      }
    }
