...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let x = arr.map(function(v){
    return v.substring(1,4)+v.substring(6,9)+v.substring(10,14);
    });
  return x;
};
...
const allHappy = (arr) => {
  ...
  arr.forEach(function(v){
    test.push(v.includes(':)'));
  })
  // console.log(test);
  if (test.includes(false)){return false;}else{return true;};
};
...
const findAnything = (arr, target) => {
  ...
  for (let i = 0; i<arr.length; i++){
    if(arr[i].includes(target))x.push(arr[i]);
  }
  return x;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  for (let i = 0; i<arr.length; i++){
    if(arr[i].includes(target)){return true;}else{return false;};
  }
};
