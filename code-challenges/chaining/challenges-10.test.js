...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let arr = data.filter(v => v.gender==='male'|| v.gender==='female').map(v=>v.name).toString().replace(/,/g,' and ');
...
let findShortest = (data) => {
  ...
  var mapped = data.map(obj =>{ 
    var heights = {};
    heights['name'] = obj.name;
    heights['height'] = parseInt(obj.height);
    return heights;
 });
 // console.log(mapped);
 let short = mapped.reduce(function(acc,v){
   v=Object.values(v);
   // console.log(v[1]);
   if(v[1]<acc){acc=v[1]}else{acc=acc};
   // console.log(acc, v);
   return acc;
 },500);
 // console.log(short);
 let almost = mapped.filter(v=>v.height===short);
 // console.log(almost);
 return almost.map(v=>v.name).toString();


 // I want to use the value of short to align with the value in the key-value pair in mapped, to select the shortest object in mapped.
