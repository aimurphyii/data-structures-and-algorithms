...
const validateEmail = (email) => {
  // Solution code here...
  if (email.match(/^\w+.\w+@\w+(.com|.net|.org)$/g)){return true;}else{return false;};
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  if (phoneNumber.match(/[a-z]/ig)){return false}else{
    if (phoneNumber.match(/^\d{10}$|^(\(\w{3}\)|\w{3})[\s.-]\w{3}[\s.-]\w{4}$|^(\(\w{3}\)|\w{3})\w{3}[\s-]\w{4}$|^\w{3}[\s-]\w{7}$/g)){return true;}else{return false;};
    }
};
